...
function applySettings (eslintConfig, settings, rootDir) {
...

  {
    ...
    if (resolvers) {
      const resolveFromRootDir = mid => {
        const result = resolveFrom.silent(rootDir, mid)
        if (!result) {
          throw new Error(`Could not resolve '${mid}' import resolver`)
        }
      ...
      if (typeof resolvers === 'string') {
        setting = resolveFromRootDir(resolvers)
      } else {
        setting = Object.keys(resolvers).reduce((acc, mid) => {
          acc[resolveFromRootDir(mid)] = resolvers[mid]
          return acc
        }, {})
      }
